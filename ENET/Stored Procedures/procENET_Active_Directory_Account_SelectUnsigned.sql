CREATE PROCEDURE [dbo].[procENET_Active_Directory_Account_SelectUnsigned]
...
WHERE (ADA.Inactive = 0) 
	...
	AND (ADA.SignedDate < '7/4/1776') 
	AND (DATEDIFF(day, ADA.LastReportedDate,
		(SELECT     TOP (1) LastReportedDate
		FROM vwENET_ACTIVE_DIRECTORY_ACCOUNT_LastReportedDate)) < 3) 
	AND datediff(day,ada.ADExpiresdate,getdate()) < 3
	OR (ADA.Inactive = 0) 
	AND (ADA.IsHidden = 0)
	AND (ADA.ServiceAccount = 0) 
	AND (ADA.SupervisorSignedDate < '7/4/1776') 
	AND (DATEDIFF(day, ADA.LastReportedDate,
		(SELECT     TOP (1) LastReportedDate
		FROM vwENET_ACTIVE_DIRECTORY_ACCOUNT_LastReportedDate)) < 3)
	AND datediff(day,ada.ADExpiresdate,getdate()) < 3
