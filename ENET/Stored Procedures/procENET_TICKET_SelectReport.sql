...
CREATE PROCEDURE [dbo].[procENET_TICKET_SelectReport]
(
	@stat int = 0,
	@sdate datetime,
	@edate datetime,
...
SET @scnt = dbo.CountSystemType(@tech)
	...
	BEGIN
		...
		BEGIN
			SELECT TKT.TicketNumber, 
				...
				CASE TKT.StatusId
					WHEN 3 THEN 0
					ELSE DateDiff(DAY,TKT.ClosedDate,TKT.OpenDate) 
				END AS DaysOpened,
				STYP.SystemDesc,
				TKT.CustomerName
			FROM TICKET AS TKT
			...
			WHERE TKT.CreatedDate BETWEEN @sdate AND @edate
				AND TKT.StatusId = 2
				AND TA.StatusId = 2
	...
	ELSE IF @ucnt > 0 AND @pcnt > 0 AND @scnt > 0 --ALL Selected
	BEGIN
		IF @stat = 2
		BEGIN
			SELECT TKT.TicketNumber, 
