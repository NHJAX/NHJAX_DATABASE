CREATE PROCEDURE [dbo].[upAssetSelect]
(
	@ast 		int = 0,
	@tech		int = 0,
	@all		int = 0,
...
FROM   MANUFACTURER MAN WITH (NOLOCK)
	...
	RIGHT OUTER JOIN DOMAIN DOM WITH (NOLOCK)
	RIGHT OUTER JOIN ASSET_TYPE TYPE WITH (NOLOCK)
	INNER JOIN ASSET AST WITH (NOLOCK)
	ON TYPE.AssetTypeId = AST.AssetTypeId 
	LEFT OUTER JOIN ASSET_SUBTYPE STYPE WITH (NOLOCK)
...
IF @sort = 'PlantPrefix'
	SELECT @sql = @sql + 'ORDER BY AST.PlantAccountPrefix '

IF @sort = 'PlantNumber'
	SELECT @sql = @sql + 'ORDER BY AST.PlantAccountNumber '

IF @sort = 'NetworkName'
	SELECT @sql = @sql + 'ORDER BY AST.NetworkName '

IF @sort = 'ManufacturerDesc'
	SELECT @sql = @sql + 'ORDER BY MAN.ManufacturerDesc '

IF @sort = 'ModelDesc'
	SELECT @sql = @sql + 'ORDER BY MOD.ModelDesc '

IF @sort = 'SerialNumber'
	SELECT @sql = @sql + 'ORDER BY AST.SerialNumber '

IF @sort = 'AcquisitionDate'
	SELECT @sql = @sql + 'ORDER BY AST.AcquisitionDate '

IF @sort = 'AssetTypeDesc'
	SELECT @sql = @sql + 'ORDER BY TYPE.AssetTypeDesc '

IF @sort = 'AssetSubTypeDesc'
	SELECT @sql = @sql + 'ORDER BY STYPE.AssetSubTypeDesc '

IF @sort = 'DispositionDesc'
	SELECT @sql = @sql + 'ORDER BY DISP.DispositionDesc '

IF @sort = 'UpdatedDate'
	SELECT @sql = @sql + 'ORDER BY AST.UpdatedDate '
