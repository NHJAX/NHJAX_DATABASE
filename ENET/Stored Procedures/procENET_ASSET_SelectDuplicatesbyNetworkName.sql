create PROCEDURE [dbo].[procENET_ASSET_SelectDuplicatesbyNetworkName] 
(
	@bOnly bit
)
AS
...
FROM PROJECT PROJ 
	RIGHT OUTER JOIN DOMAIN DOM 
	RIGHT OUTER JOIN ASSET ASSET_1 
	RIGHT OUTER JOIN ASSET_TYPE TYPE 
	INNER JOIN ASSET AST 
	ON TYPE.AssetTypeId = AST.AssetTypeId 
	LEFT OUTER JOIN ASSET_SUBTYPE STYPE 
	ON STYPE.AssetSubTypeId = AST.AssetSubtypeId 
	INNER JOIN TECHNICIAN TECH1 
    ...
    INNER JOIN vwDuplicateAssetsbyNetworkName 
	ON AST.NetworkName = vwDuplicateAssetsbyNetworkName.NetworkName 
	ON ASSET_1.AssetId = AST.SharePC 
	ON DOM.DomainId = AST.DomainId 
	LEFT OUTER JOIN DISPOSITION DISP 
	ON AST.DispositionId = DISP.DispositionId 
	LEFT OUTER JOIN DECK 
	ON AST.DeckId = DECK.DeckId 
	ON PROJ.ProjectId = AST.ProjectId 
