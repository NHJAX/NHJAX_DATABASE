CREATE PROCEDURE [dbo].[GetXRay20120815] 
(
	@PatientID	numeric,
...
SET @dDate = DATEADD(dd, 2, @EndDate)
select distinct po.OrderId, po.PatientId, p.PatientKey, p.FullName as PatientName, FamilyMemberPrefixCode as FMP, Right(SponsorSSN,4) as SSN, p.Phone,  
IsNull(loc.HospitalLocationName,'') as Clinic, IsNull(prov.ProviderName,'') as ProviderName, 
e.ExamDateTime as TakenDate, 'X-Ray' as Specimen,  
RadiologyDesc as LabTestDesc, DiagnosticCode, ResultCategoryDesc, ReportText, rr.ExamDateTime as ExamDateTime, 
e.ExamNumber, r.RadiologyCode as ProcCode, IsNull(prov.DutyPhone,'') as DutyPhone, IsNull(prov.PagerNumber,'') as PagerNumber, op.OrderPriorityDesc as Priority,
...
where po.OrderId=@OrderID and po.PatientID=@PatientID
and e.ExamDateTime between @StartDate and @EndDate
and rr.ExamDateTime between @StartDate and @dDate
