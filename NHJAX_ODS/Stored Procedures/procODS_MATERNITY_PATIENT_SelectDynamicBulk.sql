...
CREATE PROCEDURE [dbo].[procODS_MATERNITY_PATIENT_SelectDynamicBulk]
(
	@debug bit = 0,
	@mth int = 0,
	@ball bit = 0,
...
ELSE IF @srt = 'DOB'	
	...
	END
ELSE IF @srt = 'MaternityTeamDesc'	
	BEGIN
	SELECT @sql = @sql + 'ORDER BY TEAM.MaternityTeamDesc, MPAT.EDC '
	END
ELSE IF @srt = 'MaternityStatusDesc'	
	BEGIN
...
EXEC sp_executesql	@sql, @paramlist, @mth, @ball, @fpro, @tm, @stat, 
 @srt, @dir, @spat,@fmp,@enr