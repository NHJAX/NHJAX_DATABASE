...
CREATE PROCEDURE [dbo].[upODS_AHLTAMammography_Patient_Encounters] AS
...
EXEC dbo.upActivityLog 'Begin AHLTA Mammography Encounter',0,@day;
...
FROM vwSTG_AHLTA_MAMMOGRAPHY_CPT AS ACC 
...
SET @trow = 0
SET @irow = 0
EXEC dbo.upActivityLog 'Fetch AHLTA Mammography Encounter',0
...
SET @sirow = 'AHLTA Mammography Encounter Inserted: ' + CAST(@irow AS varchar(50))
SET @strow = 'AHLTA Mammography Encounter Total: ' + CAST(@trow AS varchar(50))

EXEC dbo.upActivityLog @sirow,0;
EXEC dbo.upActivityLog @strow,0;
EXEC dbo.upActivityLog 'End AHLTA Mammography Encounter',0;

--REPEAT FOR DIAGNOSIS CODES

EXEC dbo.upActivityLog 'Begin AHLTA Mammography Encounter ICD',0;
...
SELECT     
    ...
    CASE ACC.[Appointment Type]
		...
		ELSE 0
	END AS AppointmentTypeId
FROM vwSTG_AHLTA_MAMMOGRAPHY_ICD9 AS ACC 
	INNER JOIN vwODS_PATIENT_FMP AS PAT 
	ON ACC.FMP = PAT.FamilyMemberPrefixKey 
	AND ACC.SponsorSSN = PAT.SponsorSSN
...
SET @trow = 0
SET @irow = 0
EXEC dbo.upActivityLog 'Fetch AHLTA Mammography Encounter ICD',0
FETCH NEXT FROM curEnc INTO @pat,@date,@stat,@apt,@typ
...
SET @sirow = 'AHLTA Mammography Encounter ICD Inserted: ' + CAST(@irow AS varchar(50))
SET @strow = 'AHLTA Mammography Encounter ICD Total: ' + CAST(@trow AS varchar(50))

EXEC dbo.upActivityLog @sirow,0;
EXEC dbo.upActivityLog @strow,0;
EXEC dbo.upActivityLog 'End AHLTA Mammography Encounter ICD',0,@day;