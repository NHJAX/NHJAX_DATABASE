CREATE PROCEDURE dbo.upPatientUpdatebyKEY
(
	@PatientKey decimal
)
...
EXEC dbo.upActivityLog 'Begin Individual Patient',0;
...
FROM    [NHJAX-CRYSTAL].CDSS2.dbo.NED_PATIENT NP 
	LEFT OUTER JOIN PATIENT_COVERAGE PCO 
	RIGHT OUTER JOIN [NHJAX-CRYSTAL].CDSS2.dbo.vwPatientProvider PP 
	...
	RIGHT OUTER JOIN [NHJAX-CRYSTAL].CDSS2.dbo.PATIENT P 
...
WHERE P.KEY_PATIENT = @PatientKey

OPEN curPatient
SET @trow = 0
SET @irow = 0
SET @urow = 0
EXEC dbo.upActivityLog 'Fetch Individual Patient',0
FETCH NEXT FROM curPatient INTO @key_pat,@ned,@name,@sex,@dob,@age,@ssn,@pseudo,
	...
	@nedfirst,@nedmid,@pcat,@pcov,@reg,@recloc,@ben
if(@@FETCH_STATUS = 0)
BEGIN
	...
	ELSE
		...
		@nedmid,@pcat,@pcov,@reg,@recloc,@ben
	COMMIT
	END
...
SET @trow = @trow - 1
SET @surow = 'Individual Patient Updated: ' + CAST(@urow AS varchar(50))
SET @sirow = 'Individual Patient Inserted: ' + CAST(@irow AS varchar(50))
SET @strow = 'Individual Patient Total: ' + CAST(@trow AS varchar(50))
EXEC dbo.upActivityLog @surow,0;
EXEC dbo.upActivityLog @sirow,0;
EXEC dbo.upActivityLog @strow,0;
EXEC dbo.upActivityLog 'End Individual Patient',0;