...
CREATE PROCEDURE [dbo].[procODS_MATERNITY_PATIENT_SelectbyPatientId]
(
	@pat bigint
)
AS
...
--SELECTS Patient records except those
--deleted as errors. (Status:10)
BEGIN
-- 1-6-12 Added , MPAT.EDC to ORDER BY CLAUSE REE
SELECT     
...
WHERE MPAT.PatientId = @pat
AND MPAT.MaternityStatusId NOT IN (10)
ORDER BY PAT.FullName, MPAT.EDC
END
