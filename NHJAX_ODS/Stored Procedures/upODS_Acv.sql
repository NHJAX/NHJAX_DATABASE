...
CREATE PROCEDURE [dbo].[upODS_Acv] AS
...
EXEC dbo.upActivityLog 'Begin ACV',0,@day;
...
BEGIN TRANSACTION
	...
	from NHJAX_ODS.dbo.ALTERNATE_CARE_VALUE 
	Where AlternateCareValueKey = @acvkey
	SET @exists = @@RowCount
	If @exists = 0
		BEGIN
		INSERT INTO NHJAX_ODS.dbo.ALTERNATE_CARE_VALUE(AlternateCareValueKey,
	...
	ELSE
		...
		OR (@acvdesc Is Not Null AND @acvdescX Is Null)
			BEGIN
			UPDATE NHJAX_ODS.dbo.ALTERNATE_CARE_VALUE
			SET
...
EXEC dbo.upActivityLog 'End ACV',0,@day;
Drop table #Temp
