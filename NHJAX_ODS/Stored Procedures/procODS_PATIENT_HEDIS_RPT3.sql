CREATE PROCEDURE [dbo].[procODS_PATIENT_HEDIS_RPT3]
	@HospitalLocation int=0, @FlagID int=0, @SearchString nvarchar(25)=''
WITH EXEC AS CALLER
AS
DECLARE @FormattedSearchString nvarchar(25) = ''
SET @FormattedSearchString = UPPER(RTRIM(LTRIM(@SearchString)))
IF @FormattedSearchString = 'NONE' SET @FormattedSearchString = ''
		...
		ELSE
			BEGIN
				...
				FROM PATIENT AS P
				  ...
				  INNER JOIN [HOSPITAL_LOCATION] as HL ON HL.HospitalLocationId = PCM.HospitalLocationId
					AND HL.HospitalLocationId = CASE WHEN @HospitalLocation = 0 THEN HL.HospitalLocationId ELSE @HospitalLocation END	
				--WHERE UPPER(P.ODSLName) LIKE @FormattedSearchString + '%'
				WHERE UPPER(P.FullName) LIKE @FormattedSearchString + '%'
			END
	END
ELSE
	BEGIN
		IF @FormattedSearchString = ''
			BEGIN
				...
				FROM PATIENT AS P
				  ...
				  INNER JOIN [HOSPITAL_LOCATION] as HL ON HL.HospitalLocationId = PCM.HospitalLocationId
					AND HL.HospitalLocationId = CASE WHEN @HospitalLocation = 0 THEN HL.HospitalLocationId ELSE @HospitalLocation END
				WHERE P.PatientId IN (SELECT PatientId FROM PATIENT_FLAG WHERE FlagId = @FlagID )
			END
		ELSE
			BEGIN
				SELECT   P.PatientId
				...
				FROM PATIENT AS P
				  ...
				  INNER JOIN [HOSPITAL_LOCATION] as HL ON HL.HospitalLocationId = PCM.HospitalLocationId
					AND HL.HospitalLocationId = CASE WHEN @HospitalLocation = 0 THEN HL.HospitalLocationId ELSE @HospitalLocation END
				WHERE P.PatientId IN (SELECT PatientId FROM PATIENT_FLAG WHERE FlagId = @FlagID )
				--AND UPPER(P.ODSLName) LIKE @FormattedSearchString + '%'
				AND UPPER(P.FullName) LIKE @FormattedSearchString + '%'
			END
	END
