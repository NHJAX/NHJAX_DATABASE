...
BEGIN 
	...
	if	ISDATE(@date) = 1
		...
		BEGIN
			SET @time =  SUBSTRING(@date, DataLength(@date) - 3, 2) + ':' + RIGHT(@date, 2)

			if @time = '24:00'
				SET @time = '00:00';

				SET @FormattedDate = CONVERT(DATETIME, LEFT(@date, 
                      			DataLength(@date) - 5)) + CONVERT(DATETIME, @time);
		END

	/*
	SET @FormattedDate = CONVERT(DATETIME, LEFT(@date, 
                      DataLength(@date) - 5)) + CONVERT(DATETIME, SUBSTRING(@date, DataLength(@date) - 3, 
                      2) + ':' + RIGHT(@date, 2));
	*/
